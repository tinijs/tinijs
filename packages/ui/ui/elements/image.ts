...
export default class extends TiniElement {
  ...
  protected computedStyles(props: ImageStyleProps) {
    ...
    if (props.shape) {
      ...
      imgItems.push('object-fit: cover;');
    }
    ...
    if (props.innerShape) {
      imgItems.push(...this.getShapeStyles(
        props.innerShape === 'match' && props.shape
          ? props.shape
          : props.innerShape
      ));
      if (!props.shape) imgItems.push('object-fit: cover;');
    }
    if (props.fit) imgItems.push(`object-fit: ${props.fit};`);
    if (props.innerRadius) imgItems.push(`border-radius: ${parseRadiusValue(
      props.innerRadius === 'match' && props.radius
        ? props.radius
        : props.innerRadius
    )};`);
    if (props.innerOpacity) imgItems.push(`opacity: ${props.innerOpacity};`);
    if (props.innerFilter) imgItems.push(`filter: ${props.innerFilter};`);
    if (props.blend) imgItems.push(`mix-blend-mode: ${props.blend};`);
    if (props.innerClip) imgItems.push(`clip-path: ${
      props.innerClip === 'match' && props.clip
        ? props.clip
        : props.innerClip
    };`);
    if (props.innerMask) imgItems.push(`mask: ${
