...
**Pages** in TiniJS apps are special elements which purpose are to represent views or endpoints of the app. Creating and working with pages is very similar to how we would [work with elements](/framework/elements).
...
Beside the `@page()` decorator, everything else would work the same as any element. But, please note the `name: 'app-page-xxx'` property, it plays a role later when we setup the [Tini Router](https://tinijs.dev/framework/router).

## Layouts

**Layouts** in TiniJS apps are also special elements which purpose are to share common elements between pages. You can think of layouts as containers of pages.
...
Beside the `@layout()` decorator and the `<slot></slot>` in the template, everything else would work the same as any element. But, please note the `name: 'app-layout-xxx'` property, it plays a role later when we setup the [Tini Router](https://tinijs.dev/framework/router).
...
export class MyPage extends TiniElement {}
```

Please note that if you use [Tini UI](/ui), then the new page which extends `TinElement` will have access to the [base styles](/ui/native-elements) and any [shared styles](/ui/get-started#setup-ui-details). Therefore, it may change the appearance of some native elements, please do check it after the conversion.
