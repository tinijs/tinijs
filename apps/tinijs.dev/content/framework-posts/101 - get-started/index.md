...
Now, inside the project you can run `npm run dev` to start the development server. You may start development by edit the file `./app/app.ts` which is the root element of the app.

For how to work with custom elements using Lit please visit [Lit element](https://lit.dev/docs/elements/overview/), there are some differents between `LitElement` and `TiniElement`, but for now you can modify `static styles` and `render()` as you would normally do.
