...
import {
  UI_POST_ELEMENT_REGISTRY,
  UI_POST_ELEMENT_PREFIX,
} from '../../../content/ui-posts/elements.js';
...
import {AppDocPageMobileToolbarElement} from './mobile-toolbar.js';
import {AppDocPageMenuElement} from './menu.js';
import {AppDocPageTOCElement} from './toc.js';
import {AppDocPageContentElement} from './content.js';
import {AppDocPageSurroundElement} from './surround.js';
...
const elementLoader = createElementLoader(
  ...
  {
    prefixes: [UI_POST_ELEMENT_PREFIX],
  }
);

@element({
  elements: [
    AppDocPageMobileToolbarElement,
    AppDocPageMenuElement,
    AppDocPageTOCElement,
    AppDocPageContentElement,
    AppDocPageSurroundElement,
  ],
})
export class AppDocPageElement
  extends TiniElement
  implements OnCreate, OnInit, OnDestroy
{
  ...
  private _mobileToolbarRef = createRef<AppDocPageMobileToolbarElement>();
  private _menuRef = createRef<AppDocPageMenuElement>();
  private _tocRef = createRef<AppDocPageTOCElement>();
  ...
  private _globalClickHandler = (e: MouseEvent) => {
    ...
    const menuTogglerElement =
      this._mobileToolbarRef.value!.menuTogglerRef.value!;
    const tocTogglerElement =
      this._mobileToolbarRef.value!.tocTogglerRef.value!;
    // mobile menu
    if (this.mobileMenuOpened) {
      for (const element of eventPaths) {
        if (element === this._menuRef.value || element === menuTogglerElement)
          break;
        if (element === this) {
    ...
    if (this.mobileTOCOpened) {
      for (const element of eventPaths) {
        if (element === this._tocRef.value || element === tocTogglerElement)
          break;
        if (element === this) {
  ...
  private async _loadPost(postSlug: string | undefined, allPosts: DocPost[]) {
    ...
    try {
      if (!postSlug) {
        ...
        if (post?.content) {
          await elementLoader.extractAndLoad([post.content]);
        }
      }
    } catch (error) {
      post = null;
