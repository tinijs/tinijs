...
import {AppThemeSelectorElement} from '../theme-selector.js';
...
import {IconCodeElement} from '../../icons/code.js';
...
export class AppSkinEditorElement extends TiniElement {
  ...
  private modalRef = createRef<TiniModalElement>();
...
`;
  ...
  private changeColor(e: InputEvent) {
    ...
    const key = input.name;
    const value = input.value;
    return debouncer('AppSkinEditorElement:change_color', 100, () => {
  ...
  private changeValue(e: InputEvent) {
    ...
    const key = input.name;
    const value = input.value;
    return debouncer('AppSkinEditorElement:change_value', 100, () =>
      this.updateVariables([[key, value]])
    );
  }
